...
class ProductSerializer(serializers.ModelSerializer):
    ...
    category_id = rest.HashidSerializerCharField(
        source_field="catalog.Category.id", source="category.id", read_only=True
    )
    brand_id = rest.HashidSerializerCharField(
        source_field="catalog.Brand.id", source="brand.id", required=False, allow_null=True
    )

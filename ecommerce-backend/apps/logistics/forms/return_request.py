    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            Div(
             ...
             Div(
                Div('completed_at', css_class='col-span-6'),
                css_class='grid grid-cols-12 gap-4'
            ),
            Div(
                Div('reason_text', css_class='col-span-12'),
                css_class='grid grid-cols-12 gap-4'
            ),
            Div(
                Div('images', css_class='col-span-12'),
                css_class='grid grid-cols-12 gap-4'
            ),
            Submit('submit', 'Save Return', css_class='bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded')
        )

    class Meta:
        model = Return
        fields = (
            "return_number", "order", "order_item", "status", 
            "reason", "reason_text", "images", "refund_amount", 
            "reverse_shipment", "approved_at", "completed_at"
        )
        widgets = {
